	...
	public class LocalizationUtility : ModBehaviour
	{
		...
		public void RegisterLanguage(ModBehaviour mod, string languageName, string translationPath, TextTranslation.Language languageToReplace)
		{
			try
			{
				...
				// Have to fire on next update else Fixer will always be null
				// Was a breaking issue overlooked in 0.3.0
				// This isn't an ideal solution but idk what else to do	that doesn't require a rewrite
				ModHelper.Events.Unity.FireOnNextUpdate(() => customLanguage.AddTranslation(mod.ModHelper.Manifest.ModFolderPath + translationPath));
			}
			catch (Exception ex)
			{
